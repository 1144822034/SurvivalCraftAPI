...
namespace Game
{
	public class Pickable : WorldItem
	{
		...
		public virtual void UpdateMovement(float dt, ref Vector3 positionAtdt)
		{
            ...
            {
                TerrainRaycastResult? terrainRaycastResult = WrappedRaycast(Position, positionAtdt, useInteractionBoxes: false, skipAirBlocks: true, (value,_) => BlocksManager.Blocks[Terrain.ExtractContents(value)].IsCollidable_(value));
				MovingBlocksRaycastResult? movingBlocksRaycastResult = SubsystemMovingBlocks?.Raycast(Position + new Vector3(0f, 0.25f, 0f), positionAtdt + new Vector3(0f, 0.25f, 0f), true,(value,_) => BlocksManager.Blocks[Terrain.ExtractContents(value)].IsCollidable_(value));
                ...
                {
                    if (WrappedRaycast(Position, Position, useInteractionBoxes: false, skipAirBlocks: true, (value2,_) => BlocksManager.Blocks[Terrain.ExtractContents(value2)].IsCollidable_(value2)).HasValue)
                    {
            ...
            else
            {
                ...
                if (!WrappedRaycast(vector3, vector3, useInteractionBoxes: false, skipAirBlocks: true, (value,_) => BlocksManager.Blocks[Terrain.ExtractContents(value)].IsCollidable_(value)).HasValue)
                {
