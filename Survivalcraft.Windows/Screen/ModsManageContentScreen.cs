...
public class ModsManageContentScreen : Screen {
    ...
    public ModsManageContentScreen() {
        ...
        m_modsContentList.ItemClicked += delegate(object item) {
            if (item != null
                && m_modsContentList.SelectedItem == item) {
                ModItem modItem = (ModItem)item;
                if (modItem.ExternalContentEntry.Type == ExternalContentType.Directory
                    && modItem.ExternalContentEntry.Path != m_installPath) {
                    try {
                        if (modItem.ExternalContentEntry.Path != m_androidDataPath) {
                            ...
                            DialogsManager.ShowDialog(
                                null,
                                new MessageDialog(
                                    LanguageControl.Get(fName, 71),
                                    LanguageControl.Get(fName, 72) + m_androidDataPath,
                                    LanguageControl.Ok,
                                    null,
                                    null
                                )
                    ...
                    if (m_filter == StateFilter.UninstallState) {
                        ...
                        DialogsManager.ShowDialog(
                            null,
                            new MessageDialog(
                                modItem.ModInfo.Name,
                                modDescription,
                                LanguageControl.Get(fName, 60),
                                LanguageControl.Get(fName, 10),
                                delegate(MessageDialogButton result) {
                                    if (result == MessageDialogButton.Button1) {
                                        ...
                                        DialogsManager.ShowDialog(
                                            null,
                                            new MessageDialog(LanguageControl.Get(fName, 61), null, LanguageControl.Ok, null, null)
                                        );
                                    }
                                }
                            )
    ...
    public override void Enter(object[] parameters) {
        ...
        foreach (string commonPath in m_commonPaths) {
            if ((m_androidSystem && commonPath.StartsWith("android:"))
                || (!m_androidSystem && !commonPath.StartsWith("android:"))) {
        ...
        if (!m_firstEnterScreen) {
            ...
            DialogsManager.ShowDialog(
                null,
                new MessageDialog(LanguageControl.Get(fName, 14), explanation, LanguageControl.Get(fName, 15), null, null)
            );
        }
        Task.Run(
            delegate {
    ...
    public override void Update() {
        ...
        if (m_actionButton.IsClicked) {
            if (modItem != null
                ...
                if (m_filter == StateFilter.InstallState) {
                                ...
                                if (result == MessageDialogButton.Button1) {
                                    try {
                                        ...
                                        DialogsManager.ShowDialog(
                                            null,
                                            new MessageDialog(
                                                LanguageControl.Get(fName, 50),
                                                LanguageControl.Get(fName, 51) + e,
                    ...
                    if (!Storage.FileExists(installPathName)
                        && samePackmModInfo == null) {
                        Storage.CopyFile(uninstallPathName, installPathName);
                        m_installModInfo.Add(modItem.ModInfo);
                        m_installModList.Add(modItem);
                        DialogsManager.ShowDialog(
                            null,
                            new MessageDialog(LanguageControl.Get(fName, 23), fileName, LanguageControl.Get("Usual", "ok"), null, null)
                        );
                    }
                        ...
                        DialogsManager.ShowDialog(
                            ...
                            new MessageDialog(
                                ...
                                fileName + LanguageControl.Get(fName, 57),
                                LanguageControl.Get("Usual", "ok"),
                                null,
                                null
                            )
                        ...
                        if (allCount == 0) {
                            ...
                            DialogsManager.ShowDialog(
                                ...
                                new MessageDialog(
                                    ...
                                    string.Format(LanguageControl.Get(fName, 29), allCount),
        ...
        if (m_uninstallFilterButton.IsClicked
            && m_filter == StateFilter.InstallState) {
            m_filter = StateFilter.UninstallState;
            SetPath(m_uninstallPath);
            UpdateList(true);
        }
        if (m_installFilterButton.IsClicked
            && m_filter != StateFilter.InstallState) {
            m_latestScanModList.Clear();
            m_filter = StateFilter.InstallState;
            SetPath(m_installPath);
        ...
        try {
            ...
            foreach (string fileName in fileNameList) {
                ...
                if (!string.IsNullOrEmpty(extension)
                    ...
                    if (modItem.ModInfo != null
                        && modItem.ModInfo.ApiVersion.StartsWith("1.3")) {
    ...
    public void ReadyForScan(CancellableBusyDialog busyDialog) {
        ...
        busyDialog.Progress.Cancelled += delegate { m_cancelScan = true; };
    }
            ...
            if (systemPath != Storage.GetSystemPath(m_uninstallPath)) {
                foreach (string fileName in Storage.ListFileNames(validPath)) {
                    ...
                    if (!string.IsNullOrEmpty(extension)
                        ...
                        if (modInfo != null
                            && string.IsNullOrEmpty(modInfo.PackageName)) {
                            continue;
                        }
                        ...
                        if (!Storage.FileExists(uninstallPathName)) {
                            ...
                            if (modInfo != null
                                && !modInfo.ApiVersion.StartsWith("1.3")) {
                ...
                try {
                    ...
                    if (showTips) {
                        DialogsManager.ShowDialog(
                            ...
                            new MessageDialog(
                                LanguageControl.Get(fName, 4),
                                string.Format(LanguageControl.Get(fName, 41), commonPath),
    ...
    public void AddCommonPath(string path) {
        if (!m_commonPathList.Contains(path)
